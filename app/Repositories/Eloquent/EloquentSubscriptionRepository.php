...
POSTXML;
                ...
                case PaymentMethods::TYPE_PAYDUNYA:
                    try {
                        ...
                        Paydunya_Checkout_Store::setPhoneNumber("+221764592609");
                        ...
                        $invoice->addItem($item_name, 1, $plan->price, $plan->price, $item_name);
                        $invoice->setDescription($item_name);
                        $invoice->setTotalAmount($price);

                        $invoice->addCustomData("user_id", auth()->user()->id);
                        $invoice->addCustomData("payment_type", "subscriptions");
                        $invoice->addCustomData("plan_id", $plan->uid);
                        $invoice->addCustomData("price", $price);
                        $invoice->addCustomData("currency_code", $plan->currency->code);
                        if ($invoice->create()) {
                            return response()->json([
                                'status' => 'success',
